...
import { AchorCoreStaking } from "../../../target/types/achor_core_staking";
...
describe("nft-staking-core", () => {
  ...
  const program = anchor.workspace.achorCoreStaking as Program<AchorCoreStaking>;
  ...
  it("Initialize stake config", async () => {
    const tx = await program.methods.initialize(POINTS_PER_STAKED_NFT_PER_DAY, FREEZE_PERIOD_IN_DAYS)
    .accountsPartial({
